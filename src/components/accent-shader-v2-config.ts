...
/**
 * Sun v2 — same particle/beam/bolt knobs as the Sun shader, plus a
 * wave-driven depth field that's applied to each wisp (parallax shift, size,
 * brightness) inside a single fragment.
 */
export type AccentShaderV2Config = {
  speed: number
  bgColor: string
  driftSpeed: number
  cellSize: number
...
// @shader-config-start
export const ACCENT_SHADER_V2_DEFAULTS = {
  speed: 0.25,
  bgColor: "#FF5E1F",
  driftSpeed: 8,
  cellSize: 7.3,
...
export function configToShaderUniforms(
  ...
  return {
    ...
    u_waveAmp: config.waveAmp,
    u_waveScale: config.waveScale,
    u_waveSpeed: config.waveSpeed,
    u_waveTurb: config.waveTurb,
    u_depthParallax: config.depthParallax,
    u_depthSize: config.depthSize,
    u_depthBright: config.depthBright,
  }
}
