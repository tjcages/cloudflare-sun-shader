/**
 * Sun v2 fragment — same wisp / beam / lightning-bolt logic as the Sun shader,
 * plus a wave-driven depth field (inspired by the CC0 "Another flaming
 * experiment" raymarcher) baked into the wisp pass: each cell's depth shifts
 * its centre (parallax), scales its radius/bloom, and modulates its brightness,
 * so the particle field appears to undulate in 3D as it drifts upward.
 */
export const ACCENT_SHADER_V2_FRAGMENT = /* glsl */ `#version 300 es
...
uniform float u_waveTurb;
uniform float u_depthParallax;
uniform float u_depthSize;
uniform float u_depthBright;
...
// Wave-derived depth in roughly [-waveAmp, +waveAmp] for each cell.
// Combines a primary horizontal sine wave (echoing the flame shader's
// "p.x += sin(.2*t - P.x)") with a cross-axis sway and two light octaves
// of fractal turbulence.
float cellDepth(vec2 cellId, float invHeight) {
  vec2 P = (cellId + 0.5) * u_cellSize * invHeight;
  P *= u_waveScale;
  float t = u_time * u_waveSpeed;

  float z = sin(0.7 * t - P.x);
  z += 0.55 * sin(0.45 * t + P.y * 1.35);
  z += 0.4 * cos(P.x * 0.85 + P.y * 1.1 + 0.3 * t);
...
void main() {
  ...
  vec2 invView = vec2(1.0 / max(u_resolution.x / u_pixelRatio, 1.0), 1.0 / viewH);
  vec2 gridDim = vec2(u_resolution.x / u_pixelRatio, u_resolution.y / u_pixelRatio) / u_cellSize;
  float invHeight = 1.0 / viewH;
  ...
  for (int j = -1; j <= 1; j++) {
    for (int i = -1; i <= 1; i++) {
      ...
      // V2: wave-driven depth — positive = closer, negative = farther.
      float depth = cellDepth(nid, invHeight);
      // Parallax: shift the wisp centre based on depth — X dominant, plus a
      // generous Y component so the field reads as rolling waves rather than
      // a horizontal jitter.
      wispCenterPx += vec2(depth * u_depthParallax, depth * u_depthParallax * 0.55);
      // Size/brightness perspective: closer wisps grow & brighten. Clamps are
      // wide so cranking depthSize / depthBright in the dev panel actually
      // takes effect — wisps can drop to a sliver in troughs and bloom huge
      // on crests.
      float depthScale = clamp(1.0 + depth * u_depthSize, 0.1, 3.5);
      float depthBright = clamp(1.0 + depth * u_depthBright, 0.15, 2.5);
      ...
      float dotRadiusEff = dotRadius * depthScale;
      float bloomSpreadEff = bloomSpread * depthScale;
      float spread2Eff = bloomSpreadEff * bloomSpreadEff;
      float invNearSigma = 1.0 / max(u_bloomNearSigma * spread2Eff, 0.001);
      float invFarSigma = 1.0 / max(u_bloomFarSigma * spread2Eff, 0.001);

      float nBrightness =
        (mix(0.5, 1.0, nh) + revealFactor * u_brightnessRevealBoost) * depthBright;
      float wispOpacity = shimmerMix < 0.001
        ? 1.0
        : mix(1.0, wispShimmerOpacity(nid, wispCenterPx * invView), shimmerMix);

      float nDist2 = nDist * nDist;
      float nDotMask = 1.0 - smoothstep(dotRadiusEff - u_dotSoft, dotRadiusEff + u_dotSoft, nDist);
