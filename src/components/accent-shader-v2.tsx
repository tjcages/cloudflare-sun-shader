...
import {
  ACCENT_SHADER_V2_DEFAULTS,
  type AccentShaderV2Config,
  configToShaderUniforms,
} from "./accent-shader-v2-config"
import { ACCENT_SHADER_V2_DEV_FIELDS } from "./accent-shader-v2-fields"
import { ACCENT_SHADER_V2_FRAGMENT } from "./accent-shader-v2-fragment"
...
function applyQualityCaps(
  config: AccentShaderV2Config,
  tier: QualityTier,
): AccentShaderV2Config {
  return {
    ...config,
...
export function AccentShaderV2({ className }: AccentShaderV2Props) {
  const [state, setState] = useState<"loading" | "active" | "fallback">(
    "loading",
  )
  const [shaderConfig, setShaderConfig] = useState<AccentShaderV2Config>(() => ({
    ...ACCENT_SHADER_V2_DEFAULTS,
  }))
  ...
  useEffect(() => {
    return registerShaderDev({
      ...
      title: "Sun v2 shader",
      values: shaderConfig,
      defaults: { ...ACCENT_SHADER_V2_DEFAULTS },
      fields: ACCENT_SHADER_V2_DEV_FIELDS,
