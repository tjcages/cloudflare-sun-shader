/**
 * Sun v3 — "Waterfall". Keeps every Sun wisp / beam / bolt rendering detail
 * identical to v1 (screen-pixel dot mask, screen-pixel bloom, same shimmer),
 * but the wisp *grid* is sampled in polar coordinates around a virtual pivot
 * above the screen. Cells live on (angle × radius), so on screen they project
 * to concentric arcs that fan outward toward the bottom of the frame —
 * matching the cascading-streak feel of mrange's CC0 waterfall while every
 * individual particle stays Sun-sized.
 *
 *   surface unwrap   → (angle * angleScale, radius - time * drift)
 *   wisp screen pos  → pivot + (cos a, sin a) * radius
 *   dot mask         → screen-pixel distance, stretched along the radial dir
 */
export const ACCENT_SHADER_V3_FRAGMENT = /* glsl */ `#version 300 es
...
// V3 polar-waterfall
uniform float u_pivotY;          // pivot Y, in units of viewH (negative = above screen)
uniform float u_pivotX;          // pivot X offset, in units of viewW (0 = centred)
uniform float u_angleScale;      // unwrapped px per radian (controls horizontal density)
uniform float u_rotation;        // rotation of the polar layout (radians)
uniform float u_fallSwirl;       // angular drift per viewH of radius (spiral)
uniform float u_tangentStretch;  // perpendicular dot squash (>1 = thinner streaks)
uniform float u_trailLength;     // bloom elongation along the falling direction
uniform float u_depthBoost;      // brightness gain for wisps near bottom of screen
...
float boltStartCol(
  ...
  float randomCol = floor(hash(vec2(lifeId, boltId + 7.0)) * max(gridDim.x - 1.0, 1.0));
...
void main() {
  ...
  float viewH = max(u_resolution.y / u_pixelRatio, 1.0);
  float viewW = max(u_resolution.x / u_pixelRatio, 1.0);
  vec2 invView = vec2(1.0 / viewW, 1.0 / viewH);
  vec2 gridDim = vec2(viewW, viewH) / u_cellSize;
  ...
  // === Polar waterfall transform ===
  // Pivot above the screen so all on-screen pixels have a well-defined
  // (angle, radius) and the radial direction reads as "down". pivotX shifts
  // the focal point horizontally for asymmetric layouts.
  vec2 pivot = vec2(viewW * (0.5 + u_pivotX), u_pivotY * viewH);
  vec2 dp = px - pivot;
  float pxRadius = length(dp);
  float pxAngleRaw = atan(dp.y, dp.x);
  // Whole-field rotation, plus an angular spiral that grows with radius.
  float pxSwirl = u_fallSwirl * (pxRadius / viewH);
  float pxAngle = pxAngleRaw - u_rotation - pxSwirl;

  // Sample cells in (angle * scale, radius - time*drift) so wisps "fall"
  // radially outward from the pivot. Pivot above screen ⇒ on-screen wisps
  // drift downward; the polar layout produces the converge-at-top / fan-out-
  // at-bottom waterfall arcs without ever rescaling the wisps themselves.
  vec2 unwrapped = vec2(pxAngle * u_angleScale, pxRadius - u_time * u_driftSpeed);

  vec2 cellId = floor(unwrapped / u_cellSize);
  vec2 cellHalf = vec2(u_cellSize * 0.5);
  ...
  for (int j = -1; j <= 1; j++) {
    for (int i = -1; i <= 1; i++) {
      ...
      vec2 wispUnwrapped = nid * u_cellSize + cellHalf;

      // Inverse map: (angle*scale, radius - timeDrift) → screen-pixel position.
      // Undo the rotation + swirl applied to pxAngle so the cell lands at the
      // right place in screen space.
      float wispRadius = wispUnwrapped.y + u_time * u_driftSpeed;
      if (wispRadius <= 0.0) continue;
      float wispSwirl = u_fallSwirl * (wispRadius / viewH);
      float wispAngle = wispUnwrapped.x / u_angleScale + u_rotation + wispSwirl;
      vec2 radialDir = vec2(cos(wispAngle), sin(wispAngle));
      vec2 tangentDir = vec2(-radialDir.y, radialDir.x);
      vec2 wispCenterPx = pivot + radialDir * wispRadius;

      // SCREEN-PIXEL distance, decomposed along radial (falling direction) and
      // tangent (perpendicular) so each axis can be stretched independently.
      // wispStretch < 1 ⇒ longer drips; tangentStretch > 1 ⇒ thinner streaks.
      vec2 nDelta = px - wispCenterPx;
      float radialComp = dot(nDelta, radialDir);
      float tangentComp = dot(nDelta, tangentDir);
      float nDist = length(vec2(tangentComp * u_tangentStretch, radialComp * u_wispStretch));
      ...
      // Bloom uses its own stretched distance — trailLength elongates the bloom
      // along the falling direction so wisps leave motion trails.
      float bloomRadial = radialComp / max(u_trailLength, 0.05);
      float bloomDist2 = bloomRadial * bloomRadial + tangentComp * tangentComp;
      float bloomNear = exp(-bloomDist2 * invNearSigma) * u_bloomNearAmp;
      float bloomFar = useFarBloom ? exp(-bloomDist2 * invFarSigma) * u_bloomFarAmp : 0.0;
  ...
  // Optional brightness gain for wisps that landed near the bottom of the
  // screen (large radius = "closer to viewer" in the waterfall reading).
  float depthGain = 1.0;
  if (u_depthBoost > 0.001) {
    depthGain = 1.0 + u_depthBoost *
      smoothstep(viewH * 0.4, viewH * 1.4, pxRadius);
  }

  float dotIntensity = (core * u_coreAmp + wispBloom) * radialFade * depthGain;
